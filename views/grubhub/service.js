...
exports.fetchNearby = (req, res) => {
  ...
  const isLookingForBreakfast = Constants.BREAKFAST_KEYWORDS.includes(food)
  
  Utils.resolveLocation(req)
    .then(({ lat, long }) => {
      if (!lat || !long) {
        return res.json({
          status: 404,
              ...
              if (acc.length < 5) {
                ...
                if (isLookingForBreakfast) {
                  cuisinesMatch = restaurantItem.cuisines.some(cuisine => 
                    Constants.VALID_BREAKFAST_CUISINES.includes(cuisine) ||
                    restaurantItem.name.toLowerCase().includes(cuisine.toLowerCase())
                  )
                }
